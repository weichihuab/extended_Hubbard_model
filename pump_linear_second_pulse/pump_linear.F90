...
!  double precision, allocatable :: coeff_1(:), del_tmp(:), del(:,:)
  ...
  ! For pump
  double precision :: A0, A1, t_0, t_1, sigma, omega, pol_x_1, pol_y_1, pol_x_2, pol_y_2
  double precision :: Ax1, Ay1, Ax2, Ay2, phi
  integer :: x_ij, y_ij
  integer :: deg_tmp
  ...
  do
     ...
     case( 0 )
       ...
       if(str1 == 'A1') A1 = dble(input_val)
       if(str1 == 't_0') t_0 = dble(input_val)
       if(str1 == 't_1') t_1 = dble(input_val)
       if(str1 == 'sigma') sigma = dble(input_val)
       if(str1 == 'omega') omega = dble(input_val)
       if(str1 == 'pol_x_1') pol_x_1 = dble(input_val)
       if(str1 == 'pol_y_1') pol_y_1 = dble(input_val)
       if(str1 == 'pol_x_2') pol_x_2 = dble(input_val)
       if(str1 == 'pol_y_2') pol_y_2 = dble(input_val)
     case( iostat_end )
       exit
     case default
...
!!!  call MPI_Finalize ( ierr )
  ...
  if( rank .eq. 0) then
    ...
    write(*,"(A,F10.2)") 'A0 = ', A0
    write(*,"(A,F10.2)") 'A1 = ', A1
    write(*,"(A,F10.2)") 't_0 = ', t_0
    write(*,"(A,F10.2)") 't_1 = ', t_1
    write(*,"(A,F10.2)") 'sigma = ', sigma
    write(*,"(A,F10.2)") 'omega = ', omega
    write(*,"(A,F10.2)") 'pol_x_1 = ', pol_x_1
    write(*,"(A,F10.2)") 'pol_y_1 = ', pol_y_1
    write(*,"(A,F10.2)") 'pol_x_2 = ', pol_x_2
    write(*,"(A,F10.2)") 'pol_y_2 = ', pol_y_2
    write(*,*)
  end if
    ...
    do i=0, num_sites-1
      do j=0, num_sites-1
        ...
        if ((abs(T_matrix(i,j)) > small_val) .and. (btest(states_dn(dn_index),j) .eqv. .true.) .and. (btest(states_dn(dn_index),i) .eqv. .false.)) then
          ...
          col(counter) = up_index*num_states_dn+pos    ! start from 0
          ! For hopping phase
          Ax1 = A0*exp(-(t-t_0)**2/(2*sigma**2))*cos(omega*(t-t_0))*pol_x_1/sqrt(pol_x_1**2+pol_y_1**2) 
          Ay1 = A0*exp(-(t-t_0)**2/(2*sigma**2))*cos(omega*(t-t_0))*pol_y_1/sqrt(pol_x_1**2+pol_y_1**2) 
          Ax2 = A1*exp(-(t-t_1)**2/(2*sigma**2))*cos(omega*(t-t_1))*pol_x_2/sqrt(pol_x_2**2+pol_y_2**2) 
          Ay2 = A1*exp(-(t-t_1)**2/(2*sigma**2))*cos(omega*(t-t_1))*pol_y_2/sqrt(pol_x_2**2+pol_y_2**2) 
          x_ij = rx(i)-rx(j)
          y_ij = ry(i)-ry(j)
          if(x_ij == -3) x_ij =  1
          if(x_ij ==  3) x_ij = -1
